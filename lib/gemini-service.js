...
/**
 * Generates one complete response from Gemini.
 * The API key stays on the server and is never sent to the browser.
 * The result is emitted using the same token/done event contract used by
 * the existing frontend, so no frontend rewrite is required.
 */
export async function* streamOpenAIChat({ messages, systemPrompt, signal }) {
  ...
  try {
    response = await fetch(url, {
      ...
      body: JSON.stringify({
        systemInstruction: {
          parts: [{ text: systemPrompt || config.defaultSystemPrompt }],
        },
        contents,
        generationConfig: {
